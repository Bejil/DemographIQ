//
//  MB_Tutorial_ViewController.swift
//  LettroLine
//
...
public class MB_Tutorial_ViewController: MB_ViewController {
	...
	public lazy var subtitleLabel: MB_Label = {
	...
	private lazy var nextButton: MB_Button = .init() { [weak self] _ in
		
		self?.nextAction()
	}
	private lazy var elementsStackView: MB_StackView = {
	...
	}(MB_StackView(arrangedSubviews: [titleLabel, subtitleLabel, nextButton]))
	...
	private func next() {
		...
		if let sourceView = items?[currentIndex].sourceView {
			...
			UIView.animation(currentIndex > 0 ? animationDuration : 0.0) { [weak self] in
				...
				self?.positionElementsStackView(avoiding: frameInView)
		...
		UIView.animate(withDuration: animationDuration, animations: {
			...
			self.elementsStackView.alpha = 1.0
			self.elementsStackView.transform = .identity
	...
	private func positionElementsStackView(avoiding frameInView: CGRect) {
		...
		elementsStackView.snp.remakeConstraints { make in
